...
const CvCommand: CommandModule = {
  async execute(terminal: TerminalAppFacade) {
    ...
    const pdfPath = terminal.i18n.current === 'es'
      ? 'curriculum/CV_Ismail_Haddouche_Rhali_2026_ES.pdf'
      : 'curriculum/CV_Ismail_Haddouche_Rhali_2026_EN.pdf';

    content.innerHTML = `
      <h2 class="section-title">${terminal.i18n.t('commands.cv.title')}</h2>
        <div class="cv-content">
          <p class="cv-description">${terminal.i18n.t('commands.cv.desc')}</p>
          <a href="${pdfPath}" target="_blank" rel="noopener noreferrer" class="cv-download-btn">
