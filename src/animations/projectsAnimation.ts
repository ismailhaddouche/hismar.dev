...
export function init(container: HTMLElement): () => void {
  ...
  function createProjectsAnimationInstance(container: HTMLElement) {
    let isThinking = true;
  ...
  const particles: { x: number; y: number; vx: number; vy: number; life: number; decay: number }[] = [];
    ...
    const cleanup = CharacterBase.init(container, {
      ...
      getMood({ W, H, mx, my }) {
        ...
        if (isThinking) {
          return { focus: 0.95, gaze: { x: W / 2 + 35, y: H * 0.4 }, sweat: 0 };
        }
  ...
  function drawThinkingRig(
    ...
    const cloudScale = Math.max(0, blend) * 0.75;
    const bubbleX = pcx + 35;
    const bubbleY = pcy - 48;
    ...
    drawArm(ctx, state.C, rightShoulder, rightElbow, rightHand, 1);
